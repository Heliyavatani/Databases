...
FROM restBill rb
JOIN restRoom_management rrm ON rb.bill_date = rr .room_date
JOIN restStaff rs ON rrm.headwaiter = rs.staff_no
WHERE rs.first_name = 'Charles' AND rb.bill_total > 450.00;
...
-- Task 3
SELECT rb.cust_name
    FROM restBill rbWHERE rb.bill_total  (
...
FROM restBill AS rb
JOIN restRest_table AS rt ON rb.table_no = rt.table_no
