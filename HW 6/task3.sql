...
WHERE rs.first_name = 'Charles' AND rrm.room_name = 'Green' AND room_date
BETWEEN 160201 AND 160229;
...
SELECT DISTINCT rs.first_name, rs.surname
FROM restBill rb1
JOIN restBill rb2 ON rb1.table_no = rb2.table_no
JOIN restStaff rs ON rb1.waiter_no = rs.staff_no
WHERE rb2.bill_no IN (00014, 00017);

-- Task 6
SELECT DISTINCT rs.first_name, rs.surname
FROM restRoom_management rrm
JOIN restStaff rs ON rs.staff_no = rrm.headwaiter
OR rs.staff_no IN(
    SELECT waiter_no
    FROM restBill rb
...
WHERE rrm.room name = 'Blue' AND rrm.room date = 160312;
