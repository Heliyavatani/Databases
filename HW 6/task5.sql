...
-- Task 1
SELECT rs.first_name, rs.surname, rb.bill_date, COUNT(*) AS num_bills
FROM restBill rb
...
-- task 3
SELECT rrm.room_name, SUM(rb.bill_total) AS total_bill_amount
FROM restRoom_management rrm
JOIN restBill rb ON rrm.room_name = rt.room_name AND rrm.room_date = rb.bill_date
...
GROUP BY total_bill_amount DESC;
